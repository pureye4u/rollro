<script lang="ts">
  ...
  async function checkFollowerStatus(ownerId: string): Promise<void> {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        userFollowers[ownerId] =
          Array.isArray(followers) && followers.includes(userID);
      } else {
        userFollowers[ownerId] = false;
      }
    } catch (error) {
  ...
  function applyFilters() {
    ...
    if (searchKeyword.trim()) {
      ...
      routes = routes.filter((r) => r.title.toLowerCase().includes(keyword));
    }
    ...
    <div class="search-container">
      ...
      <div class="filter-chips">
        ...
        {#if selectedFilter === "my" || selectedYears.length > 0}
          <button class="filter-chip reset" on:click={resetFilters}>
    ...
    <div class="route-list-content">
      ...
      {:else if filteredRoutes.length === 0}
        <div class="empty-state">
          <Icon class="material-icons" style="font-size: 48px; color: #ccc;"
            >route</Icon
          >
          <p>여정이 없습니다</p>
        </div>
        ...
        <List class="route-list" dense>
          {#each paginatedRoutes as route}
            <RouteItem
              ...
              ownerName={route.owner ? ownerNames[route.owner] : ""}
              showOwner={true}
              on:SMUI:action={() => goto(`/route/${route.id}`)}
              on:ownerClick={(e) => {
                if (
                  route.owner &&
                  e.detail.originalEvent.currentTarget instanceof HTMLElement
                ) {
                  openUserMenu(
                    route.owner,
                    e.detail.originalEvent.currentTarget,
                  );
                }
              }}
            />
...
<style>
  .route-page-container {
    ...
    height: calc(100vh - 64px); /* Fixed height to prevent window scroll */
    padding: 16px;
  ...
  .route-list-content {
    ...
    margin-bottom: 80px;
    min-height: 200px;
    width: 100%; /* Ensure container width is constrained */
    box-sizing: border-box;
  }
